...
const PromotionModal = (props: PromotionProps) => {
  ...
  return (
    <div className='prom-container'>
      <h2 className='prom-text'>One of your pawns made it outta the trenches!</h2>
      <p className='prom-text'>Which official would you like to promote it to?</p>
      <p className='prom-text'>(Click on your desired piece)</p>
      <div className="prom-images-container">
        {pieces.map((piece, index) => (
          <div id = {getPieceId(piece as PromotionPieceType)} key={index} className="prom-image-item" onClick={() => onPromotionClick(
              fiftyMovesTracker,
              setFiftyMovesTracker,
              occupiedSquares,
              setOccupiedSquares,
            ...
            <img src={`${colorState}${getPieceId(piece as PromotionPieceType)}.png`} alt={piece} />
            <p>{piece}</p>
          </div>
        ))}
      </div>
      <button className='prom-close-button' onClick={closePromotionModal(setOpenPromotionModal)}>Return to board</button>
    </div>
  );
};
