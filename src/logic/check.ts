...
export const evaluateKingInCheck = (
  kingSquare: string,
  boardState: BoardState,
  color: PlayerColor
) => {
  const [xCoord, yCoord] = boardState[kingSquare].loc
  const kingSurroundingDirections = [
    [-1, 0],
    [1, 0],
    [0, -1],
    [0, 1],
  ...
  for (const dir of kingSurroundingDirections) {
    ...
    for (let i = 1; i < 8; i++) {
      ...
      if (isValidBoardCoordinates(targXCoord, targYCoord)) {
        const currentSquare =
          INVERTED_SQUARES[
            `${targXCoord as BoardNumbers},${targYCoord as BoardNumbers}`
          ]
  ...
  for (const knightDir of knightDirections) {
    ...
    if (isValidBoardCoordinates(targXCoordN, targYCoordN)) {
      const currentSquareN =
        INVERTED_SQUARES[
          `${targXCoordN as BoardNumbers},${targYCoordN as BoardNumbers}`
        ]
      const currentPieceN = boardState[currentSquareN].piece
      if (
        currentPieceN &&
        currentPieceN[0] !== color &&
        currentPieceN[1] === 'n'
      ) {
...
export const validMovesWhenInCheck = (
  color: PlayerColor,
  checkedSquaresAndAttackers: { [key: string]: string[] },
  boardState: BoardState,
  kingSquare: string,
  ...
  if (Object.keys(checkedSquaresAndAttackers).length === 1) {
    ...
    const attackingSquareAndSquaresToBlock = [
    ...
    attackingSquareAndSquaresToBlock.forEach((intendedSquare) => {
      ...
      if (squaresThatCanCaptureOrBlock.length) {
        squaresThatCanCaptureOrBlock.forEach((square) => {
          if (
            !pinnedSquareVals.includes(square) ||
            (pinnedSquareVals.includes(square) &&
              pinnedSquaresMap[square].validSquares.includes(intendedSquare))
          ) {
            //validMoves[square] = intendedSquare
            validMoves[square] = validMoves?.[square]
              ? validMoves[square].concat([intendedSquare])
              : [intendedSquare]
          }
        })
      }
  ...
  const squaresForbiddenForKing = allThreatenedSquares(
    color,
    boardState,
    occupiedSquares
  )
  ...
  for (const dir of SURROUNDING_DIRECTIONS) {
    ...
    if (isValidBoardCoordinates(targXCoord, targYCoord)) {
      ...
      const currentPiece = boardState[currentSquare].piece
      if (
        (!currentPiece || currentPiece[0] !== color) &&
        !squaresForbiddenForKing.includes(currentSquare)
      ) {
        validMoves[kingSquare] = validMoves?.[kingSquare]
          ? validMoves[kingSquare].concat([currentSquare])
          : [currentSquare]
      }
    }
  }
